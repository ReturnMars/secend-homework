...
import { FileSpreadsheet, Plus, Activity, Clock, ChevronRight } from 'lucide-react';
        ...
        <div className="container max-w-screen-2xl mx-auto py-10 px-4 sm:px-8 space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Manage and track your data cleaning pipelines.</p>
                </div>
                    ...
                    <DrawerContent>
                        <div className="mx-auto w-full max-w-lg">
                            <DrawerHeader>
                                ...
                                <DrawerDescription>Drag and drop your Excel/CSV file here to start cleaning.</DrawerDescription>
                            </DrawerHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card className="bg-card hover:shadow-lg transition-all duration-300 border-border/60 group relative overflow-hidden">
                    ...
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 relative z-10">
                        <CardTitle className="text-sm font-medium text-muted-foreground tracking-tight">Total Records</CardTitle>
                        <FileSpreadsheet className="h-5 w-5 text-blue-600" />
                    </CardHeader>
                    <CardContent className="relative z-10">
                        <div className="text-3xl font-bold text-foreground tracking-tighter">
                            {batches.reduce((acc, b) => acc + b.total_rows, 0).toLocaleString()}
                        </div>
                ...
                <Card className="bg-card hover:shadow-lg transition-all duration-300 border-border/60 group relative overflow-hidden">
                    ...
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 relative z-10">
                        <CardTitle className="text-sm font-medium text-muted-foreground tracking-tight">Success Rate</CardTitle>
                        <Activity className="h-5 w-5 text-green-600" />
                    </CardHeader>
                    <CardContent className="relative z-10">
                        {(() => {
                            const total = batches.reduce((acc, b) => acc + b.total_rows, 0);
                            const success = batches.reduce((acc, b) => acc + b.success_count, 0);
                ...
                <Card className="bg-card hover:shadow-lg transition-all duration-300 border-border/60 group relative overflow-hidden">
                    ...
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 relative z-10">
                        <CardTitle className="text-sm font-medium text-muted-foreground tracking-tight">Latest Import</CardTitle>
                        <Clock className="h-5 w-5 text-purple-600" />
                    </CardHeader>
                    <CardContent className="relative z-10">
                        <div className="text-3xl font-bold text-foreground tracking-tighter">
                            {batches.length > 0 ? new Date(batches[0].created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : '-'}
                        </div>
                        <div className="flex items-center text-xs text-muted-foreground mt-2">
                            <span className="opacity-80">
                                {batches.length > 0 ? new Date(batches[0].created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'No imports yet'}
                            </span>
                        </div>
                    </CardContent>
                </Card>
                ...
                <div className="border rounded-xl bg-card/50 backdrop-blur-sm shadow-sm overflow-hidden">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-muted/30 text-muted-foreground font-medium border-b border-border/50">
                            <tr>
                                <th className="py-3 px-4 w-[40%] pl-6 font-medium">Batch Identifier</th>
                                    ...
                                    <td className="py-3 px-4 pl-6">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-xs text-muted-foreground font-mono mt-0.5 opacity-70">ID: {batch.id}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="py-3 px-4">
                                        <div className="flex flex-col gap-1.5 max-w-[140px]">
                                            <div className="flex justify-between text-xs text-muted-foreground">
                                                <span>{batch.success_count.toLocaleString()} / {batch.total_rows.toLocaleString()}</span>
                                            </div>
                                            <div className="w-full h-1.5 bg-secondary rounded-full overflow-hidden">
                                                <div
                                                    className={`h-full rounded-full ${(batch.success_count / batch.total_rows) >= 0.9 ? 'bg-green-500' :
                                                        (batch.success_count / batch.total_rows) >= 0.5 ? 'bg-blue-500' : 'bg-amber-500'
                                                        }`}
                                                    style={{ width: `${batch.total_rows > 0 ? (batch.success_count / batch.total_rows) * 100 : 0}%` }}
                                                />
                                            </div>
                                        </div>
                                    </td>
                                    <td className="py-3 px-4">
                                        <Badge variant="outline" className={`font-normal border-0 px-2 py-0.5 ${batch.status === 'Completed'
                                            ? 'bg-green-500/10 text-green-700 hover:bg-green-500/20'
                                            : 'bg-secondary text-secondary-foreground hover:bg-secondary/80'
                                            }`}>
                                            <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${batch.status === 'Completed' ? 'bg-green-500' : 'bg-gray-500'
                                                }`} />
                                            {batch.status}
                                        </Badge>
                                    </td>
