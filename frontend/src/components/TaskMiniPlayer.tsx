...
const TaskMiniPlayer: React.FC<TaskMiniPlayerProps> = ({ batchId, onClose }) => {
            ...
            <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2 overflow-hidden">
                    <div className={clsx(
                        ...
                        {status === "Completed" ? <CheckCircle2 className="h-4 w-4" /> : <Loader2 className="h-4 w-4 animate-spin" />}
                    </div>
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                    <div className="grid grid-cols-2 gap-3">
                        <div className="bg-muted/30 p-2 rounded-lg">
                            <span className="text-[8px] uppercase font-black opacity-40 block mb-0.5">Speed</span>
                            <span className="text-sm font-mono font-black">
                                {status === "Completed" && metrics.speed === 0 ? "CACHED" : `${(metrics.speed / 1000).toFixed(1)}k`}
                                {!(status === "Completed" && metrics.speed === 0) && <span className="text-[10px] font-normal opacity-60 ml-0.5">r/s</span>}
                            </span>
                        </div>
                        <div className="bg-primary/5 p-2 rounded-lg">
                            <span className="text-[8px] uppercase font-black opacity-40 block mb-0.5">{status === "Completed" ? "Status" : "Time Left"}</span>
                    ...
                    <div className="space-y-1.5">
                        <div className="flex justify-between text-[10px] font-mono">
                            <span className="opacity-60">{metrics.processed.toLocaleString()} rows</span>
            ...
            {isMinimized && (
                <div className="mt-2 animate-in fade-in">
                    ...
                    <div className="flex justify-between items-center mt-1.5 px-0.5">
                        <span className="text-[9px] font-mono opacity-60">{progress}% Processed</span>
                        <div className="flex items-center gap-1">
